...
public class TriviaGUI {
	...
	private void createFileLoadingPanel() {
		...
		playButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (path != null && path.length()>0){
					try{gameData = new GameData(path);
						lastAnswer="";
						setNewQuestion();
					}
					catch (IOException exp){GUIUtils.showErrorDialog(shell,"Trivia file format error");}
				}
			}
		});
	}
	...
	private void updateQuestionPanel(String question, List<String> answers) {
		...
		for (int i = 0; i < 4; i++) {
			...
			answerButton.addSelectionListener(new SelectionAdapter() {
				@Override
				public void widgetSelected(SelectionEvent e) {
					if (gameData.wrongCount <3) {
						Button selected = (Button) e.widget;
						gameData.applyAnswer(selected.getText());
						setNewQuestion();
					}
				}
			});
		...
		fiftyFiftyButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				for (Button button: answerButtons){
					if (!(button.getText().equals(currentData[1])||button.getText().equals(currentData[rand]))){
	...
	private void setNewQuestion(){
		currentData = gameData.questions.poll();
		if (currentData != null && gameData.wrongCount <3) {
			gameData.currentCorrectAnswer= currentData[1];
			List<String> answers = Arrays.asList(currentData[1], currentData[2], currentData[3],currentData[4]);
			Collections.shuffle(answers);
			updateQuestionPanel(currentData[0],answers);
			scoreLabel.setText(""+gameData.score);
			enableButtons();
		}
		else{
			scoreLabel.setText(""+gameData.score);
			String message = String.format("Your final score is %d after %d questions.",gameData.score,gameData.attemptsCount);
			GUIUtils.showInfoDialog(shell,"GAME OVER",message);
		}
	}
	private void enableButtons(){
		passButton.setEnabled(gameData.canPass());
		fiftyFiftyButton.setEnabled(gameData.can50());
		for (Button button : answerButtons){
	...
	public class GameData {
		...
		public GameData(String path) throws IOException {
			getDataFromFile(path);
		}
		private void getDataFromFile (String path) throws IOException {
		...
		private void applyAnswer(String answer){
			gameData.attemptsCount++;
			if (answer.equals(gameData.currentCorrectAnswer)){
				gameData.wrongCount=0;
				gameData.score += GameData.CORRECT;
				lastAnswer ="Correct!";
			}
			else{
				gameData.wrongCount++;
				gameData.score += GameData.WRONG;
				lastAnswer = "Wrong...";
			}
		}
		private boolean canPass(){
			return !usedPass || score>0;
		}
		private boolean can50(){
			return !used50 || score>0;
		}
